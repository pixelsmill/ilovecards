...
export default function CardPhotoOverlay({ notion, accentColor, size, imageUrl }: TemplateProps) {
  ...
  return (
    ...
    >
      {/* Gradient overlay (color tint when no image, subtle dark scrim on top of image) */}
      {!imageUrl && (
        <>
      ...
      <div className="absolute inset-0" style={{
        background: 'linear-gradient(180deg, transparent 20%, rgba(0,0,0,0.55) 70%, rgba(0,0,0,0.85) 100%)',
      }} />
      ...
      <div className={`relative z-10 ${isThumb ? 'p-1.5' : isFull ? 'p-8' : 'p-3'}`}>
        <p
          ...
          style={{ fontFamily: "var(--font-spectral), serif", letterSpacing: '-0.3px', textShadow: '0 2px 16px rgba(0,0,0,0.5)' }}
        >
          {notion}
        </p>
      </div>
